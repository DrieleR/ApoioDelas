    ...
    <div className='min-h-screen bg-linear-to-r from-[#056881] to-[#a0dcc0] flex items-center justify-center px-4 pt-7 pb-6'>
      ...
      {/* Icone */}
        ...
        <div className='flex bg-white/20 rounded-2xl p-1'>
          <button className={`flex-1 py-2 rounded-xl text-sm font-semibold transition-all ${mode === 'login' ? 'bg-white text-[#056881] shadow' : 'text-white'}`} onClick={() => setMode('login')}>
            Entrar
          </button>
          <button className={`flex-1 py-2 rounded-xl text-sm font-semibold transition-all ${mode === 'register' ? 'bg-white text-[#056881] shadow' : 'text-white'}`} onClick={() => setMode('register')}>
            Criar Conta
          </button>
        </div>
        ...
        <div className='flex flex-col gap-4'>
          ...
          <div className='flex flex-col gap-1'>
            ...
            <div className='relative'>
              ...
              value={form.password}
              onChange={handleChange} 
              className="w-full bg-[#3ab8cc] text-white placeholder-white/70 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-white/50 pr-10"
              />
        ...
        {/* Botão principal */}
        <button onClick={handleSubmit} className="w-full bg-white/30 hover:bg-white/50 text-white font-semibold py-3 rounded-2xl transition-all duration-200 border border-white/40">
        ...
        {/* Login com Google */}
        <button className='w-full bg-white hover:bg-gray-50 text-gray-700 font-semibold py-3 rounded-2xl transition-all duration-200 flex items-center justify-center gap-2 shadow-sm text-sm'>
